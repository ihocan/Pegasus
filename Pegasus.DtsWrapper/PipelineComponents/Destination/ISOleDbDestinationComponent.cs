...
namespace Pegasus.DtsWrapper.Destination
{
    public class ISOleDbDestinationComponent : ISDestinationComponent
    {
        ...
        {
            get { return FastLoadOptions.IndexOf("TABLOCK", StringComparison.CurrentCultureIgnoreCase) >= 0 ? true : false; }
            set
            {
                _tableLock = true;
                FastLoadOptions = FastLoadOptions + (FastLoadOptions.Length == 0 ? "TABLOCK".ToUpper() : (FastLoadOptions.IndexOf("TABLOCK", StringComparison.CurrentCultureIgnoreCase) >= 0 ? "" : ",TABLOCK".ToUpper()));
            }
        }
        ...
        {
            get { return FastLoadOptions.IndexOf("CHECK_CONSTRAINTS", StringComparison.CurrentCultureIgnoreCase) >= 0 ? true : false; }
            set
            {
                _checkConstraints = true;
                FastLoadOptions = (FastLoadOptions.Length == 0 ? FastLoadOptions + "CHECK_CONSTRAINTS".ToUpper() : (FastLoadOptions.IndexOf("CHECK_CONSTRAINTS", StringComparison.CurrentCultureIgnoreCase) >= 0 ? "" : "CHECK_CONSTRAINTS".ToUpper() + "," + FastLoadOptions));
            }
        }
